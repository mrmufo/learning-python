...
while True:
    ...
    if len(direction) > 1:
        ...
        for word in directionSplit:
            if word in playerInput:
                direction = playerInput[word]
            elif word in exits[loc]:
                direction = word
    # if len(direction) > 1:
    #     for word in playerInput:
    ...
    if direction in exits[loc]:
        loc = exits[loc][direction]
    # elif direction in playerInput.keys():
